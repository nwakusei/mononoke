...
class OrderController {
  ...
  static async markOrderNotDelivered(req: Request, res: Response) {
    ...
    try {
      ...
      console.log(error);
      res.status(500).json({ message: "Erro ao marcar como não entregue!" });
    }
  }
  ...
  static async partnerCancelOrderByID(req: Request, res: Response) {
    ...
    try {
      ...
      const subtractPointsEarned = roundTo(
        customerOtakuPointPedingDecrypted - orderOtakuPointsEarnedDecrypted,
        2
      ).toString();
      ...
      const refundCustomer = roundTo(
        customerBalanceAvailableDecrypted + customerOrderCostTotalDecrypted,
        2
      ).toString();
