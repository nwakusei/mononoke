...
class OrderController {
  ...
  static async getPartnerOrderByID(req: Request, res: Response) {
    ...
    try {
      ...
      if (partner.id !== partnerIDAsString) {
        res.status(422).json({
          message: "Você não é o proprietário desse pedido, acesso negado!",
        });
        return;
      }
      ...
      const decryptedOrder = {
        ...orderObj,
        shippingCostTotal: decrypt(orderObj.shippingCostTotal),
        customerOrderCostTotal: decrypt(orderObj.customerOrderCostTotal),
        partnerCommissionOtamart: decrypt(
          orderObj.partnerCommissionOtamart.toString()
        ),
        customerOtakuPointsEarned: decrypt(orderObj.customerOtakuPointsEarned),
  ...
  static async confirmReceiptCustomerOrder(req: Request, res: Response) {
    ...
    if (!currentStatusOrder) {
      res.status(422).json({
        messsage: "Status inexistente. O status do pedido é obrigatório!",
      });
      return;
    }
    ...
    if (currentStatusOrder === "Completed" && "completed") {
      res.status(422).json({
        messsage: "Pedido já confirmado, não é possível confirmar novamente!",
      });
      return;
    }
    ...
    try {
      ...
      const partnerOtakupay = await OtakupayModel.findById(partnerOtakupayID);

      if (!partnerOtakupay || !partnerOtakupay.balancePending) {
        res.status(404).json({
          message: "Partner OtakuPay ou Balance Pending não encontrado!",
        });
        return;
      }
      ...
      if (
        Number(currentDecryptPartnerBalancePending) < Number(orderCostTotal)
      ) {
        res.status(500).json({
          message:
      ...
      console.log("COMISSÃO + CASHBACK PAGO:", decryptedCommissionOtamart);
      ...
      const customerOtakupay = await OtakupayModel.findById(customerOtakupayID);

      // Verificar se o CustomerOtakuPay e o Otaku Points Pending existem
      if (!customerOtakupay || !customerOtakupay.otakuPointsPending) {
        res.status(404).json({
          message: "Customer OtakuPay ou Otaku Points Pending não encontrado!",
        });
        return;
      }
      ...
      if (currentDecryptCustomerOtakuPointsPending === null) {
        res.status(500).json({
          message: "Erro ao descriptografar o Customer Otaku Points Pending.",
        });
        return;
      }
      ...
      if (currentDecryptCustomerOtakuPointsAvailable === null) {
        res.status(500).json({
          message: "Erro ao descriptografar o Customer Otaku Points Available.",
        });
        return;
      }
      ...
      if (
        currentDecryptCustomerOtakuPointsPending < customerOtakuPointsEarned
      ) {
        res.status(500).json({
          message:
      ...
      const newCustomerOtakuPointsPending =
        Number(currentDecryptCustomerOtakuPointsPending) -
        Number(customerOtakuPointsEarned);
      ...
      const newCustomerOtakuPointsAvailable =
        currentDecryptCustomerOtakuPointsAvailable + customerOtakuPointsEarned;
      ...
      partnerOtakupay.balanceAvailable = newEncryptedPartnerBalanceAvailable;
  ...
  static async markPacked(req: Request, res: Response) {
    ...
    if (partner.accountType !== "partner") {
      res.status(422).json({
        message: "Você não possuo autorização para realizar essa requsição!",
      });
      return;
    }
  ...
  static async markOrderNotDelivered(req: Request, res: Response) {
    const { id } = req.params;
    ...
    try {
      ...
      if (order.statusShipping === "Shipped") {
        res.status(422).json({
          message:
            "O Pedido não pode ser marcado como entregue porque está em transito!",
        });
        return;
      }
      ...
      res.status(200).json({
        message: "Pedido marcado como não entregue com sucesso!",
      });
    } catch (error) {
      console.log(error);
      res.status(500).json({
  ...
  static async partnerCancelOrderByID(req: Request, res: Response) {
    ...
    try {
      ...
      if (partnerID !== orderPartnerID) {
        res.status(422).json({
          message: "Você não possui autorização para cancelar este pedido!",
        });
        return;
      }
      ...
      const orderOtakuPointsEarnedEncrypted = order.customerOtakuPointsEarned;
      ...
      const subtractPointsEarned = roundTo(
        Number(customerOtakuPointPedingDecrypted) -
          Number(orderOtakuPointsEarnedDecrypted),
        2
      ).toString();
      ...
      const customerOrderCostTotalEncrypted = order.customerOrderCostTotal;
      ...
      const refundCustomer = roundTo(
        Number(customerBalanceAvailableDecrypted) +
          Number(customerOrderCostTotalDecrypted),
        2
      ).toString();
      ...
      const newCustomerBalanceAvailableEncrypted = encrypt(refundCustomer);
      ...
      const subtractPartnerSale = roundTo(
        Number(partnerBalancePendingDecrypted) -
          Number(customerOrderCostTotalDecrypted),
        2
      ).toString();
      ...
      const newPartnerBalancePendingEncrypted = encrypt(subtractPartnerSale);
      ...
      for (const item of order.itemsList) {
        ...
        if (!product) {
          console.log(`❌ Produto não encontrado! ID: ${item.productID}`);
          continue;
        }
        ...
        if (item.productVariation === "Sem variação") {
          ...
          const lastColonIndex = item.productVariation.lastIndexOf(":");
          ...
          const variation = product.productVariations
            ?.find((variation) => variation.title === variationTitle)
            ?.options.find((opt) => opt.name === variationName);

          if (variation) {
            variation.stock = (variation.stock || 0) + item.productQuantity;
          } else {
            console.log(
      ...
      if (partnerOtakupay) {
        partnerOtakupay.balancePending = newPartnerBalancePendingEncrypted;
      }
