...
class CustomerController {
  ...
  static async followStore(req: Request, res: Response) {
    ...
    try {
      ...
      if (!user) {
        res.status(404).json({
          message: "Faça login em uma conta cliente para seguir esta loja!",
        });
        return;
      }
      ...
      if (
        user.followingStores.some(
          (following) => following.storeID.toString() === storeID.toString()
        )
      ) {
        res.status(400).json({
  ...
  static async editCustomer(req: Request, res: Response) {
    ...
    try {
      ...
      if (customer instanceof CustomerModel) {
        ...
        const updatedUser = await CustomerModel.findById(customer._id).select(
          "-password"
        );
