...
class InterApiController {
  ...
  static async createWebhookPixInter(req: Request, res: Response) {
    ...
    try {
      ...
      const webhookData = {
        webhookUrl: "https://MEUDOMINIOAPI.com/interapi/addBalanceByPixInter",
      };
  ...
  static async getCallbacksInter(req: Request, res: Response) {
    try {
      ...
      // Consultar callbacks
      const responseConsultaCallbacks = await axios(consultaCallbacksConfig);
  ...
  static async getCobPixInter(req: Request, res: Response) {
    try {
      ...
      if (!certPath || !keyPath || !caCertPath || !clientId || !clientSecret) {
        throw new Error(
  ...
  static async addBalanceByPixInter(req: Request, res: Response) {
    try {
      ...
      if (
        ...
        (typeof callbackPayload === "object" && "pix" in callbackPayload)
      ) {
        ...
        for (const transaction of transactions) {
          ...
          if (Array.isArray(transaction.pix)) {
            ...
            for (const pixTransaction of pixTransactions) {
              ...
              console.log(
                `Transação PIX recebida: ${JSON.stringify(pixTransaction)}`
              );
              ...
              if (txid) {
                ...
                const interPixTransaction = await InterPixModel.findOne({
                  txid: txid,
                });
                ...
                if (
                  ...
                  res.status(422).json({
                    error:
                      "Transação PIX já finalizada, não é possível fazer qualquer alteração!",
                  });
                  return;
                }
                  ...
                  if (status === "CONCLUIDA") {
                    try {
                      if (interPixTransaction) {
                        ...
                        interPixTransaction.status = status;
                        await interPixTransaction.save();

                        // Obter o userID da transação PIX
                        const userID = interPixTransaction.userID;
                        ...
                        if (customerOtakupay) {
                          ...
                          const decryptedCurrentCustomerBalanceAvailable =
                            decrypt(encryptedCustomerBalanceAvalable);

                          if (
                            decryptedCurrentCustomerBalanceAvailable === null
                          ) {
                            res.status(500).json({
                              message:
                          ...
                          const newCustomerBalanceAvailable = (
                            Number(currentCustomerBalanceAvailable) +
                            Number(interPixTransaction.valor.original)
                          ).toFixed(2);
