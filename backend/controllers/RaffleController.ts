...
class RaffleController {
  ...
  static async subscriptionRaffle(req: Request, res: Response) {
    ...
    if (raffle.raffleCost > 0 && new Date(raffle.raffleDate) < currentDate) {
    ...
    try {
      ...
      if (raffleCost === 0) {
        ...
        if (
          Number(customerOtakuPointsAvailableDecrypted) < Number(raffleCost)
        ) {
          res.status(422).json({
      ...
      partnerOtakuPay.otakuPointsPending = newPartnerOtakuPointPedingEncrypted;

      const raffleAccumulatedValueEncrypted = raffle.raffleAccumulatedValue;
      ...
      const newRafflePartnerCommissionDecrypted =
        Number(rafflePartnerCommissionDecrypted) + Number(partnerComission);

      const newRafflePartnerCommissionEncrypted = encrypt(
        newRafflePartnerCommissionDecrypted.toString()
      );

      raffle.rafflePartnerCommission = newRafflePartnerCommissionEncrypted;
      ...
      raffle.winner = {
        ...
        address: {
          street:
            customer.address &&
            customer.address[0] &&
            "street" in customer.address[0]
              ? customer.address[0].street
              : "",
          complement:
            customer.address &&
            customer.address[0] &&
            "complement" in customer.address[0]
              ? customer.address[0].complement
              : "",
          neighborhood:
            customer.address &&
            customer.address[0] &&
            "neighborhood" in customer.address[0]
              ? customer.address[0].neighborhood
              : "",
          city:
            customer.address &&
            customer.address[0] &&
            "city" in customer.address[0]
              ? customer.address[0].city
              : "",
          state:
            customer.address &&
            customer.address[0] &&
            "state" in customer.address[0]
              ? customer.address[0].state
              : "",
          postalCode:
            customer.address &&
            customer.address[0] &&
            "postalCode" in customer.address[0]
              ? customer.address[0].postalCode
              : "",
        },
      };
  ...
  static async getAllRafflesByPartner(req: Request, res: Response) {
    ...
    if (partner.accountType !== "partner") {
      res.status(422).json({
        message: "Você não possui autorização para visualizar essa página!",
      });
      return;
    }
    ...
    try {
      ...
      if (
        raffles.some(
          (raffle) => raffle.partnerID.toString() !== partner._id.toString()
        )
      ) {
        res.status(403).json({
          message: "Acesso negado ao sorteio!",
      ...
      const decryptedRaffles = raffles.map((raffle) => {
        ...
        return {
          ...
          raffleAccumulatedValue: decrypt(raffleObj.raffleAccumulatedValue),

          rafflePartnerCommission: decrypt(raffleObj.rafflePartnerCommission),
        };
      });
  ...
  static async getRafflePartnerByID(req: Request, res: Response) {
    ...
    try {
      ...
      const decryptedRaffle = {
        ...
        rafflePartnerCommission: decrypt(raffle.rafflePartnerCommission),
      };
  ...
  static async getRaffleCustomerByID(req: Request, res: Response) {
    ...
    try {
      ...
      const activeTicket = raffle.registeredTickets.find(
        (ticket) => ticket.customerID.toString() === customer._id.toString()
      );
  ...
  static async getAllTicketsByCustomer(req: Request, res: Response) {
    ...
    try {
      ...
      if (!raffle) {
        res.status(404).json({
          message: "Sorteio não encontrado ou sem tickets para esse cliente!",
        });
        return;
      }
