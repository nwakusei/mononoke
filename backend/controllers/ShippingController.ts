...
class ProductController {
  ...
  static async simulateShippingMelhorEnvio(req: Request, res: Response) {
    ...
    try {
      ...
      const cepOrigem =
        partner.address.length > 0 ? partner.address[0].postalCode : undefined;
      if (!cepOrigem) {
  ...
  static async simulateShippingCorreiosModico(req: Request, res: Response) {
    ...
    try {
      ...
      if (!shippingOperator) {
        res.status(400).json({
          message: "Transportadora não é a Correios - Registro Módico!",
        });
        return;
      }
      ...
      if (!shippingEntry) {
        res.status(404).json({
          message: "Nenhuma configuração de frete encontrada para Modico.",
        });
        return;
      }
      ...
      if (partner instanceof PartnerModel) {
        ...
        if (typeof req.body.shippingConfiguration === "string") {
          try {
            ...
            console.error("Erro ao parsear shippingConfiguration:", error);
