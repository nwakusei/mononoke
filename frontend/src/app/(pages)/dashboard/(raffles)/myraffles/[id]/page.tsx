...
function MyRafflesByID() {
  ...
  }, [id, token, setMyraffle, setIsLoading, setRaffleStatus]);

  // Fetch inicial dos dados do sorteio
  useEffect(() => {
    fetchRaffleData();
  }, [id, token, fetchRaffleData]);
  ...
  }, [counter, raffleStatus, fetchRaffleData, myraffle?.winner?.customerName]);
  ...
  async function handleDraw() {
    ...
    try {
      ...
      console.error(error);
      Swal.fire({
        title: error.response?.data?.message || "Erro ao realizar sorteio",
        width: 800,
        icon: "error",
      });
    }
  ...
  async function handleDelivered() {
    setPackedLoading(true);
    try {
      const response = await api.patch(`/raffles/raffle-mark-delivered/${id}`);
              ...
              <div className="h-[402px] flex items-center justify-center mx-3 my-2">
                {myraffle?.imagesRaffle?.length > 0 && (
                  <Image
                    ...
                    alt={
                      myraffle?.productTitle || "Imagem Principal do Sorteio!"
                    }
                    width={280}
                    height={10}
                    unoptimized
          ...
          <div className="flex flex-col w-[640px] text-black py-4 pr-4">
            ...
            <div className="flex flex-row items-center gap-2">
              ...
              <span>
                {`Data do Sorteio: ${
                  myraffle?.raffleDate
                    ? format(new Date(myraffle.raffleDate), "dd/MM/yyy")
                    : ""
                }`}
              </span>
            </div>
                ...
                <div className="overflow-x-auto text-black">
                  <table className="table mb-8">
                    ...
                    <thead>
                      <tr>
                        ...
                        <th className="text-sm text-black">Custo por Ticket</th>
                  ...
                  <table className="table mb-8">
                    ...
                    <thead>
                      <tr>
                        <th className="text-sm text-black">
                          Subtotal do Pedido (Total Acumulado)
                        </th>
                        <th className="text-sm text-black">Total do Frete</th>
                        {/* <th className="text-sm text-black">
												Total do Pedido
											</th> */}
                      </tr>
          ...
          {myraffle?.statusShipping === "Not Delivered" && (
            <div className="bg-white w-[325px] p-6 border-2 border-dashed border-violet-900 rounded-md shadow-md mt-4 flex flex-col gap-2 mb-4">
              <p className="text-base font-semibold text-black mb-2">
                ...
                pedido e reembolse o comprador. Você tem 3 dias para resolver o
                problema.
              </p>
