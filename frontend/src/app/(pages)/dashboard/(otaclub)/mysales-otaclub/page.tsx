...
function MySalesOtaclubPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get("/orders/partner-otaclub-orders", {
          headers: {
            ...
            <div className="flex flex-col gap-2 ml-6 mb-6">
              ...
              <div className="overflow-x-auto">
                <table className="table">
                  ...
                  <thead>
                    <tr>
                      <th className="text-sm text-black">{/* Produtos */}</th>
                      <th className="text-sm text-black">Total do Pedido</th>
                      <th className="text-sm text-black">Status</th>
                      <th className="text-sm text-black">Comprador</th>
                      <th className="text-sm text-black">ID do Pedido</th>
                      <th></th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {mysales.length > 0 ? (
                      mysales.map((mysale) => (
                        <tr key={mysale._id}>
                                  ...
                                  <div className="font-bold">
                                    <h2 className="w-[230px] overflow-x-hidden">
                                      <span className="text-black">
                                        {mysale.itemsList[0].productTitle}
                                      </span>
                                    </h2>
                                    <span className="badge badge-info badge-sm text-white py-2">
                                      {`Total de ${
                                        mysale.itemsList.length === 1
                          ...
                          <td className="text-black">
                            <div>
                              {translateOrderStatus(mysale.statusOrder)}
                            </div>
