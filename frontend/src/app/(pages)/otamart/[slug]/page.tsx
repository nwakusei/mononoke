...
=======
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Aguarda todas as promessas
        const [productResponse, recommendedResponse, userResponse] =
          await Promise.all([productPromise, recommendedPromise, userPromise]);

        // Atualiza os estados com os dados obtidos
        setProduct(productResponse.data.product);
        setProductAdult(productResponse.data.product.productAdult);
        setRecommendedProducts(recommendedResponse.data.recommendedProducts);
        ...
        const shouldShowModal =
          productResponse.data.product.adultProduct === true &&
          (!userResponse.data || userResponse.data.viewAdultContent === false);
  ...
  const partner = partners.find((partner) => partner._id === product.partnerID);
      ...
      {showAgeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full text-center shadow-lg">
            ...
            <p className="text-gray-600 mb-6">
              Este conteúdo é destinado apenas para maiores de 18 anos. Você
              confirma que possui mais de 18 anos?
            </p>
            <div className="flex gap-4 justify-center">
              <button
                onClick={() => setShowAgeModal(false)} // Atualiza estado para fechar modal
              ...
              <div className="flex flex-row text-sm text-black mb-4 gap-1">
                ...
                <div>
                  ...
                  product?.reviews.length === 0
                    ...
                    ? `${formatHighValues(product?.reviews.length)} Avaliações`
                    : "0 Avaliações"}
                </div>
                <span>|</span>
                <div>
                  {product?.productsSold > 1
                    ? `${formatHighValues(product?.productsSold)} Vendidos`
                    : `${formatHighValues(product?.productsSold)} Vendido`}
                </div>
              </div>
              {/* Preço */}
              {selectedVariation ? (
                ...
                <div>
                  {selectedVariation.promotionalPrice > 0 ? (
                    ...
                    <h2 className="text-2xl text-primary font-semibold mb-2">
                      {Number(selectedVariation.originalPrice).toLocaleString(
                        "pt-BR",
                        {
                          style: "currency",
                          currency: "BRL",
                ...
                <div>
                  {product.productVariations.map((variation, index) => {
                    ...
                    const displayedPrices = prices.map((p) =>
                      p.promo > 0 ? p.promo : p.original
                    );
                    const lowestPrice = Math.min(...displayedPrices);
                    const highestPrice = Math.max(...displayedPrices);

                    const lowestOriginalPriceWithPromo = Math.min(
                      ...originalPricesWithPromo
                    );
                    const highestOriginalPriceWithPromo = Math.max(
                      ...originalPricesWithPromo
                    );
                      ...
                      <div key={index} className="flex flex-col my-2">
                        ...
                        <div>
                          {promotionalPrices.length > 0 ? (
                            <h2 className="text-2xl text-primary font-semibold">
                              {`${Number(lowestPrice).toLocaleString("pt-BR", {
                                style: "currency",
                                currency: "BRL",
                ...
                <div>
                  {product?.promotionalPrice > 0 ? (
                    <div>
                      <h2 className="text-2xl text-primary font-semibold">
                        {Number(product?.promotionalPrice).toLocaleString(
                          "pt-BR",
                          {
                            style: "currency",
                            currency: "BRL",
          ...
          <div className="w-full p-4 select-none">
            ...
            {partner && (
              <div className="flex flex-row gap-4">
                ...
                <div className="flex flex-col">
                  <div className="flex flex-row items-center gap-1 font-semibold text-lg">
                    <div className="text-black">{partner?.name}</div>
                    {partner?.verifiedBadge !== "" && (
                      <VerifiedBadge
                        partnerVerifiedBadge={partner?.verifiedBadge}
                      />
                    )}
                  </div>
                  <div className="flex flex-row items-center mb-2 text-black">
                    <span>{renderPartnerRatingIcons(partner?.rating)}</span>
                    <span className="text-black ml-1 mr-2">
                      {partner?.numberOfReviews === 0
                        ? "N/A"
                        : partner?.numberOfReviews === 1
                    ...
                    <span className="flex flex-row items-center gap-2 mx-2">
                      ...
                      <h1>
                        {`Seguidores: ${formatHighValues(partner?.followers)}`}
                      </h1>
                    </span>
                    <span className="text-black mb-1">|</span>
                    <div className="flex flex-row items-center gap-2 mx-2">
                  ...
                  <div className="flex flex-row gap-2">
                    ...
                    <div className="">
                      {loadingButtonId === partner.nickname ? (
                        ...
                        <button
                          onClick={() => handleClick(partner.nickname)}
                          className="btn btn-secondary w-[180px] h-[50px] rounded-md shadow-md"
                        >
                          Ver Loja
                        </button>
                      )}
                ...
                product?.reviews.map((item, index) => (
                  <div key={index} className="-mt-2">
                    <div className="flex flex-row gap-2 text-black mb-1 ml-4">
                      ...
                      <div className="flex flex-col">
                        <div>
                          <h1 className="text-sm">{item?.customerName}</h1>
                          <div className="flex flex-row items-center text-sm">
                            <span className="flex flex-row items-center gap-1">
                              <p className="flex flex-row items-center gap-1 mr-1 text-sm text-yellow-500">
                                {renderReviewRatingIcons(item?.reviewRating)}
                              </p>
                            </span>
                          </div>
                          <h3 className="text-xs mb-2">
                        ...
                        <div className="flex flex-col mb-6">
                          {/* Miniaturas */}
                          <div className="flex flex-row gap-2 mb-2">
                            {item?.imagesReview?.map((image, id) => (
                              <div
                                key={id}
                                className="w-[74px] bg-white border-black border-solid border-[1px] border-opacity-20 rounded relative shadow-md cursor-pointer"
                                onClick={() => handleImageClick(index, image)} // Passando index para a avaliação específica
                              >
                                <div className="h-[74px] flex items-center justify-center">
                                  <Image
                                    className="object-contain h-full cursor-pointer"
            ...
            <div className="flex flex-row flex-wrap gap-4 justify-center">
              {recommendedProducts.length > 0 &&
                recommendedProducts.map((recommendedProduct) => {
                  // Encontrar o parceiro correspondente com base no partnerID do produto
                  const partner = partners.find(
                    (partner) => partner._id === recommendedProduct.partnerID
                  );
                  ...
                  return (
                    <ProductAdCard
                      ...
                      product={recommendedProduct}
                      freeShipping={recommendedProduct.freeShipping}
                      productImage={`http://localhost:5000/images/products/${recommendedProduct.imagesProduct[0]}`}
                      title={recommendedProduct.productTitle}
                      originalPrice={Number(recommendedProduct.originalPrice)}
                      promotionalPrice={Number(
                        recommendedProduct.promotionalPrice
                      )}
                      price={Number(recommendedProduct.originalPrice)}
                      promoPrice={Number(recommendedProduct.promotionalPrice)}
                      cashback={cashback} // Passar o cashback para o componente ProductAdCard
