...
// Icons
import { NewPicture, Peoples, SendOne } from "@icon-park/react";
import { BsStar, BsBagCheck, BsStarHalf, BsStarFill } from "react-icons/bs";
...
function StorePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [productsRes, couponsRes, userRes, chatRes] = await Promise.all([
          api.get(`/products/getall-products-store/${foundPartner._id}`),
  ...
  const handleFollow = async () => {
    ...
    try {
      const response = await api.post(`/customers/follow-store/${partner._id}`);
          ...
          <div className="flex flex-col w-[245px]">
            ...
            <div className="flex flex-row items-center gap-1 mb-4">
              ...
              {partner?.verifiedBadge !== "" && (
                <VerifiedBadge partnerVerifiedBadge={partner?.verifiedBadge} />
              )}
            </div>
            {/* Card Store Info 2 */}
            <div className="flex flex-row items-center gap-2">
              ...
              <h1>{`Produtos: ${formatHighValues(totalProducts)}`}</h1>
            </div>
            <div className="flex flex-row items-center gap-2">
              <span>
                <Peoples size={18} />
              ...
              <h1>{`Seguidores: ${formatHighValues(followers)}`}</h1>
            </div>
                ...
                {partner?.numberOfReviews === 0
                  ...
                  : `Avaliações: ${
                      Number.isInteger(partner?.rating)
                        ? `${partner?.rating}.0`
                        : `${partner?.rating?.toFixed(1)}`
                    } (${formatHighValues(partner?.numberOfReviews)})`}
              </h1>
            </div>
            <div className="flex flex-row items-center gap-2 mb-2">
              <span>
              ...
              <h1>{`Produtos vendidos: ${formatHighValues(productsSold)}`}</h1>
            </div>
          </div>
            ...
            returnedProducts.map((returnedProduct) => {
              ...
              return (
                <ProductAdCardOtamart
                  ...
                  originalPrice={Number(returnedProduct.originalPrice)}
                  promotionalPrice={Number(returnedProduct.promotionalPrice)}
                  price={Number(returnedProduct.originalPrice)}
                  promoPrice={Number(returnedProduct.promotionalPrice)}
                  cashback={cashback}
            ...
            products.map((product) => {
              ...
              return (
                <ProductAdCardOtamart
                  ...
                  promoPrice={Number(product.promotionalPrice)}
                  cashback={cashback}
        ...
        {isChatOpen && (
          ...
          >
              ...
              >
                ...
                {viewChat.messages.map((message, index) => (
                  <div
                    key={index}
                    className={`chat ${
                      message.senderID === user?._id ? "chat-start" : "chat-end"
                    } mr-2`}
                  >
                    <div className="chat-image avatar">
                      <div className="w-10 rounded-full">
                    ...
                    <div className="chat-header flex flex-row items-center gap-2">
                      ...
                      <time className="text-xs text-gray-900 mt-1 mb-1">
                        {`${new Intl.DateTimeFormat("pt-BR", {
                          hour: "2-digit",
                          minute: "2-digit",
                        }).format(new Date(message.timestamp))} hs`}
                      </time>
                    </div>
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSendMessage(partner._id, message, imageMessage);
              }}
              ...
              {imageMessage && (
                <>
                  ...
                  <div className="flex mt-[4px] relative gap-1">
                    ...
                    {/* Botão de close */}
                    <button
                      onClick={() => setImageMessage(null)} // Limpa a imagem
              ...
              <div className="flex flex-row items-center gap-2">
                ...
                {sendButtonLoading ? (
                  ...
                  <button className="flex flex-row justify-center items-center gap-2 bg-primary w-[100px] h-[40px] transition-all ease-in duration-100 active:scale-[.97] rounded-md shadow-md mt-2">
                    <SendOne className="cursor-pointer" size={20} />
                    <span>Enviar</span>
                  </button>
                )}
              </div>
