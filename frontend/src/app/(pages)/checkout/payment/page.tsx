...
function PaymentPage() {
  ...
  useEffect(() => {
    const calcularTotalPedido = () => {
      ...
      let subtotal = productsInCart.reduce((total, product) => {
        ...
        const productQuantity = parseInt(product.quantityThisProduct, 10) || 0;
        return total + productPrice * productQuantity;
      }, 0);

      // Calcula o total do frete
      let frete = Object.values(transportadoraInfo).reduce((total, info) => {
      ...
      const descontoTotalProdutos = productsInCart.reduce(
        (totalDesconto, product) => {
          const productPrice = parseFloat(product.productPrice) || 0;
          const productPriceTotal = parseFloat(product.productPriceTotal) || 0;
          const productQuantity =
            parseInt(product.quantityThisProduct, 10) || 0;
          if (productPriceTotal !== productPrice) {
            const desconto = productPrice * productQuantity - productPriceTotal;
      ...
      const totalDiscountAmount = couponsStorage.reduce(
        (total, coupon) => total + (parseFloat(coupon.discountAmount) || 0),
        0
      );
  ...
  useEffect(() => {
    const fetchPaymentIntent = async () => {
      try {
        const response = await api.post("/otakupay/create-payment-intent");
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex flex-col justify-between gap-4 border-[1px] border-black border-opacity-20 bg-white w-[690px] min-h-[100px] p-4 rounded-md shadow-md">
              <div className="flex flex-row gap-4 text-black">
                <div>
                  <h1 className="text-lg">Escolha o método de pagamento</h1>
                </div>
              </div>
                ...
                {clientSecret &&
                  visiblePaymentContent === "creditCardContent" && (
                    <CheckoutCreditCardInstallmentsContent
                      orderTotalCost={Number(totalPedido.toFixed(2))}
