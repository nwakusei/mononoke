...
function CartPage() {
          ...
          if (filteredProducts.length > 0) {
            ...
            filteredProducts.forEach((product) => {
              ...
              if (!productInfo[partnerID]) {
                ...
                productInfo[partnerID].productPriceTotal += productPriceTotal;
                productInfo[partnerID].quantityThisProduct +=
                  quantityThisProduct;
              }
  ...
  const increaseQuantity = async (productId) => {
    try {
      ...
      console.log("Dados descriptografados (antes do parse):", decryptedData);
      ...
      console.log("Dados descriptografados (após o parse):", productsInCart);
      ...
      if (index !== -1) {
        ...
        console.log("Carrinho criptografado antes de salvar:", encryptedCart);
        ...
        if (filteredProducts.length > 0) {
          ...
          filteredProducts.forEach((product) => {
            ...
            if (!productInfo[partnerID]) {
              ...
              productInfo[partnerID].productPrice += productPrice;
              productInfo[partnerID].productPriceTotal += productPriceTotal;
              productInfo[partnerID].quantityThisProduct += quantityThisProduct;
            }
              ...
              Object.entries(
                ...
                >
                  {partnerProducts.map((productInCart) => (
                    <div
                      key={`${productInCart.productID}-${
                        productInCart.productVariations?.[0]?.optionID ||
                        "no-var"
                      }`}
                        ...
                        <div className="flex flex-row gap-4">
                            ...
                            <Image
                              ...
                              src={`https://mononokebucket.s3.us-east-1.amazonaws.com/${
                                productInCart.productVariations?.length > 0
                                  ? productInCart.productVariations[0].imageUrl
                                  : productInCart.imageProduct
                              }`}
                              alt={productInCart.productTitle}
                              width={100}
                              height={100}
                              unoptimized
                            />
                          ...
                          <div>
                            ...
                            <div className="flex flex-row items-center text-black mt-2 gap-2">
                              <button
                                onClick={() =>
                                  decreaseQuantity(productInCart.productID)
                                }
                              ...
                              <input
                                ...
                                type="text"
                                value={productInCart.quantityThisProduct}
                                readOnly
                              />
                              <button
                                onClick={() =>
                                  increaseQuantity(productInCart.productID)
                                }
                                className="flex items-center justify-center w-[30px] h-[30px] select-none font-mono"
                              >
                                <h1 className="px-3 py-1 shadow-md shadow-gray-500/50 bg-black text-white rounded cursor-pointer active:scale-[.97]">
                        ...
                        <div className="flex flex-row items-center gap-4">
                          <div>
                            <h1 className="text-black">
                              ...
                              x {productInCart.quantityThisProduct}
                            </h1>
                          </div>
                  ...
                  {Object.entries(transportadoraInfo).map(
                    ([partnerID, info]) => {
                      const matchingProduct = partnerProducts.find(
