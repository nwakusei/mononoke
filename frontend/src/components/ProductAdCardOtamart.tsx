...
function ProductAdCardOtamart({
  ...
  const calculateDiscountPercentage = (product) => {
    ...
    const calculatePercentage = (original, promotional) => {
      ...
      const discountPercentage = ((original - promotional) / original) * 100;
      return Math.round(discountPercentage);
    };

    if (product.productVariations?.length > 0) {
      const discounts = product.productVariations.flatMap((variation) =>
        (variation.options ?? [])
          .filter((option) => option.promotionalPrice > 0)
          .map((option) =>
            calculatePercentage(option.originalPrice, option.promotionalPrice)
          )
          .filter((discount) => discount !== null)
      );

      if (discounts.length === 0) {
        return ""; // Nenhuma promoção válida
      }

      // Use spread apenas se discounts não estiver vazio
  ...
  return (
    <div className="bg-white w-[254px] flex flex-col rounded-md relative pb-2 shadow-md select-none">
      ...
      <div className="flex flex-col justify-center mx-4 -mt-2">
        <div>
          <h1 className="font-semibold text-base text-black line-clamp-2 whitespace-normal min-h-[46px]">
            {title}
          </h1>
        </div>
        <div>
                ...
                {(product.productVariations ?? []).map((variation, index) => {
                  ...
                  const lowestPrice =
                    displayedPrices.length > 0
                      ? Math.min(...displayedPrices)
                      : 0;
                  const highestPrice =
                    displayedPrices.length > 0
                      ? Math.max(...displayedPrices)
                      : 0;

                  const lowestOriginalPriceWithPromo =
                    originalPricesWithPromo.length > 0
                      ? Math.min(...originalPricesWithPromo)
                      : 0;
