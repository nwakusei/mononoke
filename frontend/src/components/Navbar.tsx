...
import { Currency, Blockchain } from "@icon-park/react";
import {
  LuTags,
  LuLayoutDashboard,
  LuSettings,
...
function Navbar() {
  ...
  const { cart, setCart, subtotal, setSubtotal } = useContext(CheckoutContext);
  ...
  useEffect(() => {
    async function atualizarCarrinho() {
      try {
        ...
        const updatedProducts = await Promise.all(
          productsInCart.map(async (item) => {
            try {
              ...
              if (
                ...
                const matchedOption =
                  productData.productVariations[0]?.options.find(
                    (opt) => opt.name === selectedVariation.name
                  );
                if (matchedOption) {
                  updatedPrice =
                  ...
                  updatedImage = productData.productImages?.[0] || updatedImage;
                }
              } else {
                updatedPrice =
                  productData.promotionalPrice > 0
              ...
              return {
                ...item,
                productTitle: productData.productTitle || item.productTitle,
        ...
        localStorage.setItem("productsInCart", encryptData(updatedProducts));
                  ...
                  <div className="card-body">
                    ...
                    <div className="card-actions">
                      <button className="btn btn-secondary text-white shadow-md btn-block">
                        <Link href="/checkout/cart">Ver Carrinho</Link>
                      </button>
                    </div>
                  </div>
                </div>
                      ...
                      <>
                        ...
                        {user.accountType === "partner" ? (
                          <Image
                            ...
                            unoptimized
                            priority
                            onLoadingComplete={() => setIsImageLoaded(true)} // Atualiza o estado quando a imagem é carregada
                            style={{
