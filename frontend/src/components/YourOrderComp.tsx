      ...
      try {
        ...
        if (decryptedString) {
          ...
          try {
            ...
            console.error("Erro ao tentar fazer JSON.parse:", parseError);
            console.error("String que causou erro:", decryptedString);
          }
        } else {
          console.error(
        ...
        console.error("Erro ao processar os produtos do carrinho:", error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    const calcularTotalPedido = () => {
      let subtotal = productsInfo.reduce(
        (total, productInCart) => total + productInCart.productPriceTotal,
        0
      );
  ...
  useEffect(() => {
    const calcularTotalPedido = () => {
      let subtotal = productsInfo.reduce(
        (total, productInCart) =>
          total +
          productInCart.productPrice * productInCart.quantityThisProduct, // Corrigido para "quantityThisProduct"
        0
      );
    ...
    calcularTotalPedido();
  }, [productsInfo, shippingInfo, couponApplied, calculateTotalFrete]);
    ...
    try {
      ...
      if (cupomInfo) {
        ...
        if (couponsStorageEncrypted) {
          const decryptedCouponsStorage = decryptData(couponsStorageEncrypted);
        ...
        if (!cupomJaAplicado) {
          ...
          if (isCouponApplicable) {
            const produtosComPartnerIDCorreto = productsInfo.map((product) => {
              if (product.partnerID === cupomInfo.partnerID) {
                const descontoPercentual = cupomInfo.discountPercentage / 100;
                const desconto = product.productPriceTotal * descontoPercentual;
            ...
            localStorage.setItem("productsInCart", encryptedProductsInCart);
      ...
      {productsInfo.length > 0 && (
        <div>
          <div className="text-black flex flex-col w-[400px] min-h-[250px] border-[1px] border-black border-opacity-20 bg-white p-4 rounded-md shadow-md">
            <div>
              <h1 className="text-lg font-semibold mb-6">Seu Pedido</h1>
                ...
                productsInfo.map((productInCart, index) => (
                  ...
                  >
                    <h2>
                      ...
                      {productInCart.productVariations?.length > 0 &&
                        productInCart.productVariations.map((variation) => (
                          <h2
                    ...
                    <h2>
                      {productInCart.productPrice
                        ? productInCart.productPrice.toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          })
                        : "Preço indisponível"}
            ...
            <div>
              <div className="flex justify-between mb-1">
                <h2 className="flex items-center justify-center gap-1">
                  ...
                  >
                    <FiInfo className="animate-pulse" size={16} />
                  </div>
                </h2>
                <h2>
              ...
              <div className="flex justify-between mb-1">
                ...
                <div>
                  <h2>
                    {calculateTotalFrete().toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  </h2>
