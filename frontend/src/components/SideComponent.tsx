...
function SideComponent({ selectedVariation }) {
  ...
  function handleSelected(
    ...
    setSelectedTransportadora((prevState) => {
      const deselectedItems = Object.keys(prevState).reduce(
        (acc, key) => ({
          ...acc,
          [key]: key === transportadoraIDSimulacao ? !prevState[key] : false,
        }),
        {}
      );
      return {
        ...deselectedItems,
        [transportadoraIDSimulacao]: !prevState[transportadoraIDSimulacao],
  ...
  function handleAddProductInCart(quantity, product, selectedTransportadora) {
    ...
    if (hasVariations) {
      const isVariationSelected = product.productVariations.every(
        (variation) => selectedVariations && selectedVariations[variation._id]
      );

      if (!isVariationSelected) {
        toast.info("Selecione uma variação antes de adicionar ao carrinho!");
        return;
      }
    }
    ...
    if (productsInCart) {
      try {
        ...
        console.error("Erro ao processar o carrinho do localStorage:", error);
        productsInCart = [];
      }
    } else {
      productsInCart = [];
    ...
    if (existingProduct) {
      const totalQuantity = existingProduct.quantityThisProduct + quantity;
      existingProduct.quantityThisProduct = Math.min(totalQuantity, stock);

      if (totalQuantity > stock) {
        toast.warning("Você atingiu o limite de estoque para este produto!");
      }
  ...
  async function handleSimulateShipping(cep: number, quantity: number) {
    ...
    if (selectedVariation) {
      const variationPrice =
        selectedVariation.promotionalPrice || selectedVariation.originalPrice;
      productPrice =
    ...
    try {
      ...
      try {
        ...
        fretesRecebidos = [...fretesRecebidos, ...responseMelhorEnvio.data];
      } catch (error) {
      ...
      if (
        ...
        try {
          const responseModico = await api.post("/shippings/simulate-modico", {
            productID: product._id,
            cepDestino: cep,
            weight: product.weight,
            height: product.height,
          ...
          if (responseModico?.data) {
            fretesRecebidos = [...fretesRecebidos, ...responseModico.data];
          } else {
      ...
      setTransportadoras(
        fretesRecebidos.sort((a, b) => Number(a.price) - Number(b.price))
      );
    } finally {
        ...
        <div className="bg-white border-black border-solid border-[1px] border-opacity-20 rounded-lg shadow-md mr-2 mb-2">
          <div className="px-4 mb-2">
            ...
            <button
              ...
              onClick={() =>
                handleBuyNow(quantity, product, selectedTransportadora)
              }
            >
        ...
        <div className="mr-2">
          ...
          {product.preOrder === true ? (
            <div className="flex flex-row text-sm justify-center items-center bg-sky-500 p-2 gap-3 rounded-md shadow-md mb-2">
              ...
              <h1>Encomenda (envio em {product.daysShipping} dias)</h1>
            </div>
          ) : (
            <></>
          )}
            ...
            product.freeShippingRegion === customerStateAddress ? (
              <div>
                ...
                <div className="flex flex-row justify-between items-center gap-2 mb-1">
                  ...
                  >
                    <FiInfo className="animate-pulse" size={18} />
                  </div>
                </div>
              </div>
            ) : partner?.address?.length === 0 ? (
              <div className="flex flex-row justify-between items-center gap-2 mb-1">
                ...
                >
                  <FiInfo className="animate-pulse text-red-500" size={18} />
                </div>
              </div>
            ) : partner ? (
              <div>
              ...
              <div className="flex flex-row justify-center items-center gap-2 mb-1">
                <SkeletonTheme baseColor="#202020" highlightColor="#444">
                  <Skeleton circle={false} height={40} width={250} />
                </SkeletonTheme>
              </div>
            )}
            ...
            {transportadoras.length > 0 ? (
              transportadoras.map((transportadora) => (
                <div
                  ...
                  className={`${
                    selectedTransportadora[transportadora.id]
                ...
                >
                  <div className="flex flex-row justify-between items-center gap-2 mb-1">
                    ...
                    <h2>
                      {transportadora.price &&
                        parseFloat(transportadora.price).toLocaleString(
                          "pt-BR",
                          {
                            style: "currency",
                            currency: "BRL",
                  ...
                  <div className="flex flex-row justify-between">
                    ...
                    <h2 className="text-sm">
                      {`≅ ${
                        product.daysShipping + transportadora.delivery_time
                      } dias`}
                    </h2>
                  </div>
                </div>
